...
| `/journey` | End-to-end orchestrator: keeps a persistent journey state in `documents/journey_state.md`, tells you what is required vs optional next, and gives explicit handoffs to specialized commands. |
...
| `/apply <url or text>` | The full application pipeline: evaluate fit → register a drafted tracker row + archive posting text → draft CV + cover letter → reviewer agent critiques → revise → compile to PDF → ATS check → present. See [How /apply works](#how-apply-works) below. |
...
| `/status` | Read-only dashboard for application state: grouped overview, company-specific status, or stale follow-up view (`/status --stale 14`). |
...
3. **Register draft state** — create/update a `drafted` row in `job_search_tracker.csv` and save the posting text to `documents/applications/<company>_<role>/job_posting.md`.
4. **Draft** a tailored CV and cover letter in LaTeX.
5. **Spawn a reviewer agent** with fresh context that researches the company and critiques the drafts.
6. **Revise** based on that feedback.
7. **Compile and inspect** both PDFs — `lualatex` for the CV, `xelatex` for the cover letter. Claude reads the rendered pages and iterates until the CV is exactly 2 pages with no orphaned entries, and the cover letter is exactly 1 page with a visible signature and consistent fonts.
8. **ATS-check the CV** — extract the PDF's text layer and verify it the way an applicant-tracking system would: contact details present as literal text, no garbled glyphs, sane reading order, honest keyword coverage against the posting (gaps stay visible, never stuffed).
9. **Present** the final output with a verification checklist.
...
If you want a single guide across the whole process, start with `/journey start`.

What it does:

- Creates and maintains `documents/journey_state.md`
- Uses state-first guidance (so it does not rescan everything each time)
- Separates required steps from optional upgrades
- Gives explicit handoff cards (what to run, why now, what to prepare, what to expect)

Typical usage:

1. `/journey start`
2. Run the suggested command (for example `/setup`)
3. Return with `/journey update`
4. Repeat `/journey next` until you reach outcomes/calibration

Template reference: [docs/journey-state-template.md](journey-state-template.md).
...
4. Check `/status` to see what is drafted, submitted, stale, or resolved.
5. Submit the ones you're happy with.
6. `/outcome` as responses come in — interview requests, rejections, silence, all logged.
7. Interview scheduled → `/interview` the day before.
8. Enough outcomes recorded → re-run `/setup` to let real signal calibrate the fit framework.
